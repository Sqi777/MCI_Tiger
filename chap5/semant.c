...
struct expty transVar(S_table venv, S_table tenv, A_var v){
    switch (v->kind){
        ...
        case A_fieldVar: {
			struct expty exp = transVar(venv, tenv, v->u.field.var); // recursion from here
			Ty_ty t = exp.ty;
			if (t->kind != Ty_record) {
    ...
    switch(d->kind) {
        ...
        case A_functionDec:{
            ...
            for(a_fdl = d->u.function; a_fdl; a_fdl = a_fdl->tail){
                ...
                S_enter(venv, a_fdl->head->name, E_FunEntry(formalTys, resultTy));
            }
            setRset(s);
            for(a_fdl = d->u.function; a_fdl; a_fdl = a_fdl->tail){
                if(!setPut(s, a_fdl->head->name))
                ...
                {A_fieldList l; 
                 ...
                 for(l = a_fdl->head->params; l; l = l->tail, t = t->tail){
                    S_enter(venv, l->head->name, E_VarEntry(t->head));
                 }
                }
                struct expty exp = transExp(venv, tenv, a_fdl->head->body);
